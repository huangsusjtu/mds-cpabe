...
extern const char* TYPE_A_PARAMS;
extern const char* pub_file;
extern const char* msk_file ;
...
void attr_from_file()
{
	...
	while(f>>buf)
	{
		...
		element_init_G2(temppub.Version, pubkey->p);
		...
		element_pow_zn( temppub.Version, pubkey->g2 , tempmas.ver );   //public version
...
/*init for public secret infomation*/
	...
	pairing_apply(pubkey->gt, pubkey->g1, pubkey->g2 ,pubkey->p) ;
...
{
	ofstream f(pub_file, std::ofstream::out);
